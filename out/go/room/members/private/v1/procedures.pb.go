// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.6
// 	protoc        v3.21.12
// source: room/members/private/v1/procedures.proto

package memberspb

import (
	v13 "gitlab.com/evemeta/zdk/pb/out/go/gateway/sfus/private/v1"
	v1 "gitlab.com/evemeta/zdk/pb/out/go/gateway/transponders/private/v1"
	v12 "gitlab.com/evemeta/zdk/pb/out/go/room/connections/private/v1"
	v11 "gitlab.com/evemeta/zdk/pb/out/go/room/streams/private/v1"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type CountArgument struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Cache         bool                   `protobuf:"varint,1,opt,name=cache,proto3" json:"cache,omitempty"`
	Query         *Query                 `protobuf:"bytes,2,opt,name=query,proto3" json:"query,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CountArgument) Reset() {
	*x = CountArgument{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CountArgument) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CountArgument) ProtoMessage() {}

func (x *CountArgument) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CountArgument.ProtoReflect.Descriptor instead.
func (*CountArgument) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{0}
}

func (x *CountArgument) GetCache() bool {
	if x != nil {
		return x.Cache
	}
	return false
}

func (x *CountArgument) GetQuery() *Query {
	if x != nil {
		return x.Query
	}
	return nil
}

type CountRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Arguments     []*CountArgument       `protobuf:"bytes,1,rep,name=arguments,proto3" json:"arguments,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CountRequest) Reset() {
	*x = CountRequest{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CountRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CountRequest) ProtoMessage() {}

func (x *CountRequest) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CountRequest.ProtoReflect.Descriptor instead.
func (*CountRequest) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{1}
}

func (x *CountRequest) GetArguments() []*CountArgument {
	if x != nil {
		return x.Arguments
	}
	return nil
}

type CountResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Total         int64                  `protobuf:"varint,1,opt,name=total,proto3" json:"total,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CountResponse) Reset() {
	*x = CountResponse{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CountResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CountResponse) ProtoMessage() {}

func (x *CountResponse) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CountResponse.ProtoReflect.Descriptor instead.
func (*CountResponse) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{2}
}

func (x *CountResponse) GetTotal() int64 {
	if x != nil {
		return x.Total
	}
	return 0
}

type RangeArgument struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Cache         bool                   `protobuf:"varint,1,opt,name=cache,proto3" json:"cache,omitempty"`
	Query         *Query                 `protobuf:"bytes,2,opt,name=query,proto3" json:"query,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RangeArgument) Reset() {
	*x = RangeArgument{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RangeArgument) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RangeArgument) ProtoMessage() {}

func (x *RangeArgument) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RangeArgument.ProtoReflect.Descriptor instead.
func (*RangeArgument) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{3}
}

func (x *RangeArgument) GetCache() bool {
	if x != nil {
		return x.Cache
	}
	return false
}

func (x *RangeArgument) GetQuery() *Query {
	if x != nil {
		return x.Query
	}
	return nil
}

type RangeRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Arguments     []*RangeArgument       `protobuf:"bytes,1,rep,name=arguments,proto3" json:"arguments,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RangeRequest) Reset() {
	*x = RangeRequest{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RangeRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RangeRequest) ProtoMessage() {}

func (x *RangeRequest) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RangeRequest.ProtoReflect.Descriptor instead.
func (*RangeRequest) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{4}
}

func (x *RangeRequest) GetArguments() []*RangeArgument {
	if x != nil {
		return x.Arguments
	}
	return nil
}

type RangeResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Chunks        []*Chunk               `protobuf:"bytes,1,rep,name=chunks,proto3" json:"chunks,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RangeResponse) Reset() {
	*x = RangeResponse{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RangeResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RangeResponse) ProtoMessage() {}

func (x *RangeResponse) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RangeResponse.ProtoReflect.Descriptor instead.
func (*RangeResponse) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{5}
}

func (x *RangeResponse) GetChunks() []*Chunk {
	if x != nil {
		return x.Chunks
	}
	return nil
}

type SelectArgument struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Cache         bool                   `protobuf:"varint,1,opt,name=cache,proto3" json:"cache,omitempty"`
	Query         *Query                 `protobuf:"bytes,2,opt,name=query,proto3" json:"query,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SelectArgument) Reset() {
	*x = SelectArgument{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SelectArgument) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SelectArgument) ProtoMessage() {}

func (x *SelectArgument) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SelectArgument.ProtoReflect.Descriptor instead.
func (*SelectArgument) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{6}
}

func (x *SelectArgument) GetCache() bool {
	if x != nil {
		return x.Cache
	}
	return false
}

func (x *SelectArgument) GetQuery() *Query {
	if x != nil {
		return x.Query
	}
	return nil
}

type SelectRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Arguments     []*SelectArgument      `protobuf:"bytes,1,rep,name=arguments,proto3" json:"arguments,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SelectRequest) Reset() {
	*x = SelectRequest{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SelectRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SelectRequest) ProtoMessage() {}

func (x *SelectRequest) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SelectRequest.ProtoReflect.Descriptor instead.
func (*SelectRequest) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{7}
}

func (x *SelectRequest) GetArguments() []*SelectArgument {
	if x != nil {
		return x.Arguments
	}
	return nil
}

type SelectResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Members       []*Member              `protobuf:"bytes,1,rep,name=members,proto3" json:"members,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SelectResponse) Reset() {
	*x = SelectResponse{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SelectResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SelectResponse) ProtoMessage() {}

func (x *SelectResponse) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SelectResponse.ProtoReflect.Descriptor instead.
func (*SelectResponse) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{8}
}

func (x *SelectResponse) GetMembers() []*Member {
	if x != nil {
		return x.Members
	}
	return nil
}

type CreateArgument struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	UserId        string                 `protobuf:"bytes,2,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	RoomId        string                 `protobuf:"bytes,3,opt,name=room_id,json=roomId,proto3" json:"room_id,omitempty"`
	Status        Status                 `protobuf:"varint,4,opt,name=status,proto3,enum=room.members.private.v1.Status" json:"status,omitempty"`
	Permissions   []Permission           `protobuf:"varint,8,rep,packed,name=permissions,proto3,enum=room.members.private.v1.Permission" json:"permissions,omitempty"`
	Metadata      map[string]string      `protobuf:"bytes,5,rep,name=metadata,proto3" json:"metadata,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	EnterTime     int64                  `protobuf:"varint,6,opt,name=enter_time,json=enterTime,proto3" json:"enter_time,omitempty"`
	LeaveTime     int64                  `protobuf:"varint,7,opt,name=leave_time,json=leaveTime,proto3" json:"leave_time,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateArgument) Reset() {
	*x = CreateArgument{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateArgument) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateArgument) ProtoMessage() {}

func (x *CreateArgument) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateArgument.ProtoReflect.Descriptor instead.
func (*CreateArgument) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{9}
}

func (x *CreateArgument) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *CreateArgument) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *CreateArgument) GetRoomId() string {
	if x != nil {
		return x.RoomId
	}
	return ""
}

func (x *CreateArgument) GetStatus() Status {
	if x != nil {
		return x.Status
	}
	return StatusUnknown
}

func (x *CreateArgument) GetPermissions() []Permission {
	if x != nil {
		return x.Permissions
	}
	return nil
}

func (x *CreateArgument) GetMetadata() map[string]string {
	if x != nil {
		return x.Metadata
	}
	return nil
}

func (x *CreateArgument) GetEnterTime() int64 {
	if x != nil {
		return x.EnterTime
	}
	return 0
}

func (x *CreateArgument) GetLeaveTime() int64 {
	if x != nil {
		return x.LeaveTime
	}
	return 0
}

type CreateRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Arguments     []*CreateArgument      `protobuf:"bytes,1,rep,name=arguments,proto3" json:"arguments,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateRequest) Reset() {
	*x = CreateRequest{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateRequest) ProtoMessage() {}

func (x *CreateRequest) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateRequest.ProtoReflect.Descriptor instead.
func (*CreateRequest) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{10}
}

func (x *CreateRequest) GetArguments() []*CreateArgument {
	if x != nil {
		return x.Arguments
	}
	return nil
}

type CreateResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Members       []*Member              `protobuf:"bytes,1,rep,name=members,proto3" json:"members,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateResponse) Reset() {
	*x = CreateResponse{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateResponse) ProtoMessage() {}

func (x *CreateResponse) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateResponse.ProtoReflect.Descriptor instead.
func (*CreateResponse) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{11}
}

func (x *CreateResponse) GetMembers() []*Member {
	if x != nil {
		return x.Members
	}
	return nil
}

type CreateMutation struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	Foundation     *CreateArgument        `protobuf:"bytes,1,opt,name=foundation,proto3" json:"foundation,omitempty"`
	Condition      *CreateCondition       `protobuf:"bytes,2,opt,name=condition,proto3" json:"condition,omitempty"`
	Transient      []*Transient           `protobuf:"bytes,3,rep,name=transient,proto3" json:"transient,omitempty"`
	BeginTimestamp int64                  `protobuf:"varint,4,opt,name=begin_timestamp,json=beginTimestamp,proto3" json:"begin_timestamp,omitempty"`
	CloseTimestamp int64                  `protobuf:"varint,5,opt,name=close_timestamp,json=closeTimestamp,proto3" json:"close_timestamp,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *CreateMutation) Reset() {
	*x = CreateMutation{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateMutation) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateMutation) ProtoMessage() {}

func (x *CreateMutation) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateMutation.ProtoReflect.Descriptor instead.
func (*CreateMutation) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{12}
}

func (x *CreateMutation) GetFoundation() *CreateArgument {
	if x != nil {
		return x.Foundation
	}
	return nil
}

func (x *CreateMutation) GetCondition() *CreateCondition {
	if x != nil {
		return x.Condition
	}
	return nil
}

func (x *CreateMutation) GetTransient() []*Transient {
	if x != nil {
		return x.Transient
	}
	return nil
}

func (x *CreateMutation) GetBeginTimestamp() int64 {
	if x != nil {
		return x.BeginTimestamp
	}
	return 0
}

func (x *CreateMutation) GetCloseTimestamp() int64 {
	if x != nil {
		return x.CloseTimestamp
	}
	return 0
}

type CreateCondition struct {
	state                      protoimpl.MessageState `protogen:"open.v1"`
	CreateTransponderMutations []*v1.CreateMutation   `protobuf:"bytes,1,rep,name=create_transponder_mutations,json=createTransponderMutations,proto3" json:"create_transponder_mutations,omitempty"`
	unknownFields              protoimpl.UnknownFields
	sizeCache                  protoimpl.SizeCache
}

func (x *CreateCondition) Reset() {
	*x = CreateCondition{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateCondition) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateCondition) ProtoMessage() {}

func (x *CreateCondition) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateCondition.ProtoReflect.Descriptor instead.
func (*CreateCondition) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{13}
}

func (x *CreateCondition) GetCreateTransponderMutations() []*v1.CreateMutation {
	if x != nil {
		return x.CreateTransponderMutations
	}
	return nil
}

type CreateTransaction struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Mutations     []*CreateMutation      `protobuf:"bytes,1,rep,name=mutations,proto3" json:"mutations,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateTransaction) Reset() {
	*x = CreateTransaction{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateTransaction) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateTransaction) ProtoMessage() {}

func (x *CreateTransaction) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateTransaction.ProtoReflect.Descriptor instead.
func (*CreateTransaction) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{14}
}

func (x *CreateTransaction) GetMutations() []*CreateMutation {
	if x != nil {
		return x.Mutations
	}
	return nil
}

type UpdateArgument struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Query         *Query                 `protobuf:"bytes,1,opt,name=query,proto3" json:"query,omitempty"`
	UserId        *OptionalString        `protobuf:"bytes,2,opt,name=user_id,json=userId,proto3,oneof" json:"user_id,omitempty"`
	RoomId        *OptionalString        `protobuf:"bytes,3,opt,name=room_id,json=roomId,proto3,oneof" json:"room_id,omitempty"`
	Metadata      *OptionalMap           `protobuf:"bytes,4,opt,name=metadata,proto3,oneof" json:"metadata,omitempty"`
	Permissions   *Permission            `protobuf:"varint,5,opt,name=permissions,proto3,enum=room.members.private.v1.Permission,oneof" json:"permissions,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateArgument) Reset() {
	*x = UpdateArgument{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateArgument) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateArgument) ProtoMessage() {}

func (x *UpdateArgument) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateArgument.ProtoReflect.Descriptor instead.
func (*UpdateArgument) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{15}
}

func (x *UpdateArgument) GetQuery() *Query {
	if x != nil {
		return x.Query
	}
	return nil
}

func (x *UpdateArgument) GetUserId() *OptionalString {
	if x != nil {
		return x.UserId
	}
	return nil
}

func (x *UpdateArgument) GetRoomId() *OptionalString {
	if x != nil {
		return x.RoomId
	}
	return nil
}

func (x *UpdateArgument) GetMetadata() *OptionalMap {
	if x != nil {
		return x.Metadata
	}
	return nil
}

func (x *UpdateArgument) GetPermissions() Permission {
	if x != nil && x.Permissions != nil {
		return *x.Permissions
	}
	return PermissionUnknown
}

type UpdateRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Arguments     []*UpdateArgument      `protobuf:"bytes,1,rep,name=arguments,proto3" json:"arguments,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateRequest) Reset() {
	*x = UpdateRequest{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateRequest) ProtoMessage() {}

func (x *UpdateRequest) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateRequest.ProtoReflect.Descriptor instead.
func (*UpdateRequest) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{16}
}

func (x *UpdateRequest) GetArguments() []*UpdateArgument {
	if x != nil {
		return x.Arguments
	}
	return nil
}

type UpdateResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Members       []*Member              `protobuf:"bytes,1,rep,name=members,proto3" json:"members,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateResponse) Reset() {
	*x = UpdateResponse{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[17]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateResponse) ProtoMessage() {}

func (x *UpdateResponse) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[17]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateResponse.ProtoReflect.Descriptor instead.
func (*UpdateResponse) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{17}
}

func (x *UpdateResponse) GetMembers() []*Member {
	if x != nil {
		return x.Members
	}
	return nil
}

type UpdateMutation struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	Foundation     *UpdateArgument        `protobuf:"bytes,1,opt,name=foundation,proto3" json:"foundation,omitempty"`
	Transient      []*Transient           `protobuf:"bytes,2,rep,name=transient,proto3" json:"transient,omitempty"`
	BeginTimestamp int64                  `protobuf:"varint,3,opt,name=begin_timestamp,json=beginTimestamp,proto3" json:"begin_timestamp,omitempty"`
	CloseTimestamp int64                  `protobuf:"varint,4,opt,name=close_timestamp,json=closeTimestamp,proto3" json:"close_timestamp,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *UpdateMutation) Reset() {
	*x = UpdateMutation{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateMutation) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateMutation) ProtoMessage() {}

func (x *UpdateMutation) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateMutation.ProtoReflect.Descriptor instead.
func (*UpdateMutation) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{18}
}

func (x *UpdateMutation) GetFoundation() *UpdateArgument {
	if x != nil {
		return x.Foundation
	}
	return nil
}

func (x *UpdateMutation) GetTransient() []*Transient {
	if x != nil {
		return x.Transient
	}
	return nil
}

func (x *UpdateMutation) GetBeginTimestamp() int64 {
	if x != nil {
		return x.BeginTimestamp
	}
	return 0
}

func (x *UpdateMutation) GetCloseTimestamp() int64 {
	if x != nil {
		return x.CloseTimestamp
	}
	return 0
}

type UpdateTransaction struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Mutations     []*UpdateMutation      `protobuf:"bytes,1,rep,name=mutations,proto3" json:"mutations,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateTransaction) Reset() {
	*x = UpdateTransaction{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[19]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateTransaction) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateTransaction) ProtoMessage() {}

func (x *UpdateTransaction) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[19]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateTransaction.ProtoReflect.Descriptor instead.
func (*UpdateTransaction) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{19}
}

func (x *UpdateTransaction) GetMutations() []*UpdateMutation {
	if x != nil {
		return x.Mutations
	}
	return nil
}

type DeleteArgument struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Query         *Query                 `protobuf:"bytes,1,opt,name=query,proto3" json:"query,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteArgument) Reset() {
	*x = DeleteArgument{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[20]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteArgument) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteArgument) ProtoMessage() {}

func (x *DeleteArgument) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[20]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteArgument.ProtoReflect.Descriptor instead.
func (*DeleteArgument) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{20}
}

func (x *DeleteArgument) GetQuery() *Query {
	if x != nil {
		return x.Query
	}
	return nil
}

type DeleteRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Arguments     []*DeleteArgument      `protobuf:"bytes,1,rep,name=arguments,proto3" json:"arguments,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteRequest) Reset() {
	*x = DeleteRequest{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[21]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteRequest) ProtoMessage() {}

func (x *DeleteRequest) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[21]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteRequest.ProtoReflect.Descriptor instead.
func (*DeleteRequest) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{21}
}

func (x *DeleteRequest) GetArguments() []*DeleteArgument {
	if x != nil {
		return x.Arguments
	}
	return nil
}

type DeleteResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Members       []*Member              `protobuf:"bytes,1,rep,name=members,proto3" json:"members,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteResponse) Reset() {
	*x = DeleteResponse{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[22]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteResponse) ProtoMessage() {}

func (x *DeleteResponse) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[22]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteResponse.ProtoReflect.Descriptor instead.
func (*DeleteResponse) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{22}
}

func (x *DeleteResponse) GetMembers() []*Member {
	if x != nil {
		return x.Members
	}
	return nil
}

type DeleteMutation struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	Foundation     *DeleteArgument        `protobuf:"bytes,1,opt,name=foundation,proto3" json:"foundation,omitempty"`
	Condition      *DeleteCondition       `protobuf:"bytes,2,opt,name=condition,proto3" json:"condition,omitempty"`
	Transient      []*Transient           `protobuf:"bytes,3,rep,name=transient,proto3" json:"transient,omitempty"`
	BeginTimestamp int64                  `protobuf:"varint,4,opt,name=begin_timestamp,json=beginTimestamp,proto3" json:"begin_timestamp,omitempty"`
	CloseTimestamp int64                  `protobuf:"varint,5,opt,name=close_timestamp,json=closeTimestamp,proto3" json:"close_timestamp,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *DeleteMutation) Reset() {
	*x = DeleteMutation{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[23]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteMutation) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteMutation) ProtoMessage() {}

func (x *DeleteMutation) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[23]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteMutation.ProtoReflect.Descriptor instead.
func (*DeleteMutation) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{23}
}

func (x *DeleteMutation) GetFoundation() *DeleteArgument {
	if x != nil {
		return x.Foundation
	}
	return nil
}

func (x *DeleteMutation) GetCondition() *DeleteCondition {
	if x != nil {
		return x.Condition
	}
	return nil
}

func (x *DeleteMutation) GetTransient() []*Transient {
	if x != nil {
		return x.Transient
	}
	return nil
}

func (x *DeleteMutation) GetBeginTimestamp() int64 {
	if x != nil {
		return x.BeginTimestamp
	}
	return 0
}

func (x *DeleteMutation) GetCloseTimestamp() int64 {
	if x != nil {
		return x.CloseTimestamp
	}
	return 0
}

type DeleteCondition struct {
	state                      protoimpl.MessageState `protogen:"open.v1"`
	DeleteTransponderMutations []*v1.DeleteMutation   `protobuf:"bytes,1,rep,name=delete_transponder_mutations,json=deleteTransponderMutations,proto3" json:"delete_transponder_mutations,omitempty"`
	unknownFields              protoimpl.UnknownFields
	sizeCache                  protoimpl.SizeCache
}

func (x *DeleteCondition) Reset() {
	*x = DeleteCondition{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[24]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteCondition) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteCondition) ProtoMessage() {}

func (x *DeleteCondition) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[24]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteCondition.ProtoReflect.Descriptor instead.
func (*DeleteCondition) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{24}
}

func (x *DeleteCondition) GetDeleteTransponderMutations() []*v1.DeleteMutation {
	if x != nil {
		return x.DeleteTransponderMutations
	}
	return nil
}

type DeleteTransaction struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Mutations     []*DeleteMutation      `protobuf:"bytes,1,rep,name=mutations,proto3" json:"mutations,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteTransaction) Reset() {
	*x = DeleteTransaction{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[25]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteTransaction) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteTransaction) ProtoMessage() {}

func (x *DeleteTransaction) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[25]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteTransaction.ProtoReflect.Descriptor instead.
func (*DeleteTransaction) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{25}
}

func (x *DeleteTransaction) GetMutations() []*DeleteMutation {
	if x != nil {
		return x.Mutations
	}
	return nil
}

type KickArgument struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Query         *Query                 `protobuf:"bytes,1,opt,name=query,proto3" json:"query,omitempty"`
	Reason        string                 `protobuf:"bytes,2,opt,name=reason,proto3" json:"reason,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *KickArgument) Reset() {
	*x = KickArgument{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[26]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *KickArgument) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*KickArgument) ProtoMessage() {}

func (x *KickArgument) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[26]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use KickArgument.ProtoReflect.Descriptor instead.
func (*KickArgument) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{26}
}

func (x *KickArgument) GetQuery() *Query {
	if x != nil {
		return x.Query
	}
	return nil
}

func (x *KickArgument) GetReason() string {
	if x != nil {
		return x.Reason
	}
	return ""
}

type KickRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Arguments     []*KickArgument        `protobuf:"bytes,1,rep,name=arguments,proto3" json:"arguments,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *KickRequest) Reset() {
	*x = KickRequest{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[27]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *KickRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*KickRequest) ProtoMessage() {}

func (x *KickRequest) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[27]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use KickRequest.ProtoReflect.Descriptor instead.
func (*KickRequest) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{27}
}

func (x *KickRequest) GetArguments() []*KickArgument {
	if x != nil {
		return x.Arguments
	}
	return nil
}

type KickResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *KickResponse) Reset() {
	*x = KickResponse{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[28]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *KickResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*KickResponse) ProtoMessage() {}

func (x *KickResponse) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[28]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use KickResponse.ProtoReflect.Descriptor instead.
func (*KickResponse) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{28}
}

type KickMutation struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	Foundation     *KickArgument          `protobuf:"bytes,1,opt,name=foundation,proto3" json:"foundation,omitempty"`
	Condition      *KickCondition         `protobuf:"bytes,2,opt,name=condition,proto3" json:"condition,omitempty"`
	BeginTimestamp int64                  `protobuf:"varint,3,opt,name=begin_timestamp,json=beginTimestamp,proto3" json:"begin_timestamp,omitempty"`
	CloseTimestamp int64                  `protobuf:"varint,4,opt,name=close_timestamp,json=closeTimestamp,proto3" json:"close_timestamp,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *KickMutation) Reset() {
	*x = KickMutation{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[29]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *KickMutation) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*KickMutation) ProtoMessage() {}

func (x *KickMutation) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[29]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use KickMutation.ProtoReflect.Descriptor instead.
func (*KickMutation) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{29}
}

func (x *KickMutation) GetFoundation() *KickArgument {
	if x != nil {
		return x.Foundation
	}
	return nil
}

func (x *KickMutation) GetCondition() *KickCondition {
	if x != nil {
		return x.Condition
	}
	return nil
}

func (x *KickMutation) GetBeginTimestamp() int64 {
	if x != nil {
		return x.BeginTimestamp
	}
	return 0
}

func (x *KickMutation) GetCloseTimestamp() int64 {
	if x != nil {
		return x.CloseTimestamp
	}
	return 0
}

type KickCondition struct {
	state                         protoimpl.MessageState    `protogen:"open.v1"`
	UpdateMutations               []*UpdateMutation         `protobuf:"bytes,1,rep,name=update_mutations,json=updateMutations,proto3" json:"update_mutations,omitempty"`
	DeleteStreamMutations         []*v11.DeleteMutation     `protobuf:"bytes,2,rep,name=delete_stream_mutations,json=deleteStreamMutations,proto3" json:"delete_stream_mutations,omitempty"`
	DeleteConnectionMutations     []*v12.DeleteMutation     `protobuf:"bytes,3,rep,name=delete_connection_mutations,json=deleteConnectionMutations,proto3" json:"delete_connection_mutations,omitempty"`
	DeleteTransponderMutations    []*v1.DeleteMutation      `protobuf:"bytes,4,rep,name=delete_transponder_mutations,json=deleteTransponderMutations,proto3" json:"delete_transponder_mutations,omitempty"`
	DisconnectConnectionMutations []*v13.DisconnectMutation `protobuf:"bytes,5,rep,name=disconnect_connection_mutations,json=disconnectConnectionMutations,proto3" json:"disconnect_connection_mutations,omitempty"`
	unknownFields                 protoimpl.UnknownFields
	sizeCache                     protoimpl.SizeCache
}

func (x *KickCondition) Reset() {
	*x = KickCondition{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[30]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *KickCondition) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*KickCondition) ProtoMessage() {}

func (x *KickCondition) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[30]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use KickCondition.ProtoReflect.Descriptor instead.
func (*KickCondition) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{30}
}

func (x *KickCondition) GetUpdateMutations() []*UpdateMutation {
	if x != nil {
		return x.UpdateMutations
	}
	return nil
}

func (x *KickCondition) GetDeleteStreamMutations() []*v11.DeleteMutation {
	if x != nil {
		return x.DeleteStreamMutations
	}
	return nil
}

func (x *KickCondition) GetDeleteConnectionMutations() []*v12.DeleteMutation {
	if x != nil {
		return x.DeleteConnectionMutations
	}
	return nil
}

func (x *KickCondition) GetDeleteTransponderMutations() []*v1.DeleteMutation {
	if x != nil {
		return x.DeleteTransponderMutations
	}
	return nil
}

func (x *KickCondition) GetDisconnectConnectionMutations() []*v13.DisconnectMutation {
	if x != nil {
		return x.DisconnectConnectionMutations
	}
	return nil
}

type KickTransaction struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Mutations     []*KickMutation        `protobuf:"bytes,1,rep,name=mutations,proto3" json:"mutations,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *KickTransaction) Reset() {
	*x = KickTransaction{}
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[31]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *KickTransaction) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*KickTransaction) ProtoMessage() {}

func (x *KickTransaction) ProtoReflect() protoreflect.Message {
	mi := &file_room_members_private_v1_procedures_proto_msgTypes[31]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use KickTransaction.ProtoReflect.Descriptor instead.
func (*KickTransaction) Descriptor() ([]byte, []int) {
	return file_room_members_private_v1_procedures_proto_rawDescGZIP(), []int{31}
}

func (x *KickTransaction) GetMutations() []*KickMutation {
	if x != nil {
		return x.Mutations
	}
	return nil
}

var File_room_members_private_v1_procedures_proto protoreflect.FileDescriptor

const file_room_members_private_v1_procedures_proto_rawDesc = "" +
	"\n" +
	"(room/members/private/v1/procedures.proto\x12\x17room.members.private.v1\x1a#room/members/private/v1/enums.proto\x1a&room/members/private/v1/entities.proto\x1a'room/members/private/v1/optionals.proto\x1a(room/streams/private/v1/procedures.proto\x1a,room/connections/private/v1/procedures.proto\x1a(gateway/sfus/private/v1/procedures.proto\x1a0gateway/transponders/private/v1/procedures.proto\"[\n" +
	"\rCountArgument\x12\x14\n" +
	"\x05cache\x18\x01 \x01(\bR\x05cache\x124\n" +
	"\x05query\x18\x02 \x01(\v2\x1e.room.members.private.v1.QueryR\x05query\"T\n" +
	"\fCountRequest\x12D\n" +
	"\targuments\x18\x01 \x03(\v2&.room.members.private.v1.CountArgumentR\targuments\"%\n" +
	"\rCountResponse\x12\x14\n" +
	"\x05total\x18\x01 \x01(\x03R\x05total\"[\n" +
	"\rRangeArgument\x12\x14\n" +
	"\x05cache\x18\x01 \x01(\bR\x05cache\x124\n" +
	"\x05query\x18\x02 \x01(\v2\x1e.room.members.private.v1.QueryR\x05query\"T\n" +
	"\fRangeRequest\x12D\n" +
	"\targuments\x18\x01 \x03(\v2&.room.members.private.v1.RangeArgumentR\targuments\"G\n" +
	"\rRangeResponse\x126\n" +
	"\x06chunks\x18\x01 \x03(\v2\x1e.room.members.private.v1.ChunkR\x06chunks\"\\\n" +
	"\x0eSelectArgument\x12\x14\n" +
	"\x05cache\x18\x01 \x01(\bR\x05cache\x124\n" +
	"\x05query\x18\x02 \x01(\v2\x1e.room.members.private.v1.QueryR\x05query\"V\n" +
	"\rSelectRequest\x12E\n" +
	"\targuments\x18\x01 \x03(\v2'.room.members.private.v1.SelectArgumentR\targuments\"K\n" +
	"\x0eSelectResponse\x129\n" +
	"\amembers\x18\x01 \x03(\v2\x1f.room.members.private.v1.MemberR\amembers\"\xa0\x03\n" +
	"\x0eCreateArgument\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x17\n" +
	"\auser_id\x18\x02 \x01(\tR\x06userId\x12\x17\n" +
	"\aroom_id\x18\x03 \x01(\tR\x06roomId\x127\n" +
	"\x06status\x18\x04 \x01(\x0e2\x1f.room.members.private.v1.StatusR\x06status\x12E\n" +
	"\vpermissions\x18\b \x03(\x0e2#.room.members.private.v1.PermissionR\vpermissions\x12Q\n" +
	"\bmetadata\x18\x05 \x03(\v25.room.members.private.v1.CreateArgument.MetadataEntryR\bmetadata\x12\x1d\n" +
	"\n" +
	"enter_time\x18\x06 \x01(\x03R\tenterTime\x12\x1d\n" +
	"\n" +
	"leave_time\x18\a \x01(\x03R\tleaveTime\x1a;\n" +
	"\rMetadataEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"V\n" +
	"\rCreateRequest\x12E\n" +
	"\targuments\x18\x01 \x03(\v2'.room.members.private.v1.CreateArgumentR\targuments\"K\n" +
	"\x0eCreateResponse\x129\n" +
	"\amembers\x18\x01 \x03(\v2\x1f.room.members.private.v1.MemberR\amembers\"\xb5\x02\n" +
	"\x0eCreateMutation\x12G\n" +
	"\n" +
	"foundation\x18\x01 \x01(\v2'.room.members.private.v1.CreateArgumentR\n" +
	"foundation\x12F\n" +
	"\tcondition\x18\x02 \x01(\v2(.room.members.private.v1.CreateConditionR\tcondition\x12@\n" +
	"\ttransient\x18\x03 \x03(\v2\".room.members.private.v1.TransientR\ttransient\x12'\n" +
	"\x0fbegin_timestamp\x18\x04 \x01(\x03R\x0ebeginTimestamp\x12'\n" +
	"\x0fclose_timestamp\x18\x05 \x01(\x03R\x0ecloseTimestamp\"\x84\x01\n" +
	"\x0fCreateCondition\x12q\n" +
	"\x1ccreate_transponder_mutations\x18\x01 \x03(\v2/.gateway.transponders.private.v1.CreateMutationR\x1acreateTransponderMutations\"Z\n" +
	"\x11CreateTransaction\x12E\n" +
	"\tmutations\x18\x01 \x03(\v2'.room.members.private.v1.CreateMutationR\tmutations\"\x9c\x03\n" +
	"\x0eUpdateArgument\x124\n" +
	"\x05query\x18\x01 \x01(\v2\x1e.room.members.private.v1.QueryR\x05query\x12E\n" +
	"\auser_id\x18\x02 \x01(\v2'.room.members.private.v1.OptionalStringH\x00R\x06userId\x88\x01\x01\x12E\n" +
	"\aroom_id\x18\x03 \x01(\v2'.room.members.private.v1.OptionalStringH\x01R\x06roomId\x88\x01\x01\x12E\n" +
	"\bmetadata\x18\x04 \x01(\v2$.room.members.private.v1.OptionalMapH\x02R\bmetadata\x88\x01\x01\x12J\n" +
	"\vpermissions\x18\x05 \x01(\x0e2#.room.members.private.v1.PermissionH\x03R\vpermissions\x88\x01\x01B\n" +
	"\n" +
	"\b_user_idB\n" +
	"\n" +
	"\b_room_idB\v\n" +
	"\t_metadataB\x0e\n" +
	"\f_permissions\"V\n" +
	"\rUpdateRequest\x12E\n" +
	"\targuments\x18\x01 \x03(\v2'.room.members.private.v1.UpdateArgumentR\targuments\"K\n" +
	"\x0eUpdateResponse\x129\n" +
	"\amembers\x18\x01 \x03(\v2\x1f.room.members.private.v1.MemberR\amembers\"\xed\x01\n" +
	"\x0eUpdateMutation\x12G\n" +
	"\n" +
	"foundation\x18\x01 \x01(\v2'.room.members.private.v1.UpdateArgumentR\n" +
	"foundation\x12@\n" +
	"\ttransient\x18\x02 \x03(\v2\".room.members.private.v1.TransientR\ttransient\x12'\n" +
	"\x0fbegin_timestamp\x18\x03 \x01(\x03R\x0ebeginTimestamp\x12'\n" +
	"\x0fclose_timestamp\x18\x04 \x01(\x03R\x0ecloseTimestamp\"Z\n" +
	"\x11UpdateTransaction\x12E\n" +
	"\tmutations\x18\x01 \x03(\v2'.room.members.private.v1.UpdateMutationR\tmutations\"F\n" +
	"\x0eDeleteArgument\x124\n" +
	"\x05query\x18\x01 \x01(\v2\x1e.room.members.private.v1.QueryR\x05query\"V\n" +
	"\rDeleteRequest\x12E\n" +
	"\targuments\x18\x01 \x03(\v2'.room.members.private.v1.DeleteArgumentR\targuments\"K\n" +
	"\x0eDeleteResponse\x129\n" +
	"\amembers\x18\x01 \x03(\v2\x1f.room.members.private.v1.MemberR\amembers\"\xb5\x02\n" +
	"\x0eDeleteMutation\x12G\n" +
	"\n" +
	"foundation\x18\x01 \x01(\v2'.room.members.private.v1.DeleteArgumentR\n" +
	"foundation\x12F\n" +
	"\tcondition\x18\x02 \x01(\v2(.room.members.private.v1.DeleteConditionR\tcondition\x12@\n" +
	"\ttransient\x18\x03 \x03(\v2\".room.members.private.v1.TransientR\ttransient\x12'\n" +
	"\x0fbegin_timestamp\x18\x04 \x01(\x03R\x0ebeginTimestamp\x12'\n" +
	"\x0fclose_timestamp\x18\x05 \x01(\x03R\x0ecloseTimestamp\"\x84\x01\n" +
	"\x0fDeleteCondition\x12q\n" +
	"\x1cdelete_transponder_mutations\x18\x01 \x03(\v2/.gateway.transponders.private.v1.DeleteMutationR\x1adeleteTransponderMutations\"Z\n" +
	"\x11DeleteTransaction\x12E\n" +
	"\tmutations\x18\x01 \x03(\v2'.room.members.private.v1.DeleteMutationR\tmutations\"\\\n" +
	"\fKickArgument\x124\n" +
	"\x05query\x18\x01 \x01(\v2\x1e.room.members.private.v1.QueryR\x05query\x12\x16\n" +
	"\x06reason\x18\x02 \x01(\tR\x06reason\"R\n" +
	"\vKickRequest\x12C\n" +
	"\targuments\x18\x01 \x03(\v2%.room.members.private.v1.KickArgumentR\targuments\"\x0e\n" +
	"\fKickResponse\"\xed\x01\n" +
	"\fKickMutation\x12E\n" +
	"\n" +
	"foundation\x18\x01 \x01(\v2%.room.members.private.v1.KickArgumentR\n" +
	"foundation\x12D\n" +
	"\tcondition\x18\x02 \x01(\v2&.room.members.private.v1.KickConditionR\tcondition\x12'\n" +
	"\x0fbegin_timestamp\x18\x03 \x01(\x03R\x0ebeginTimestamp\x12'\n" +
	"\x0fclose_timestamp\x18\x04 \x01(\x03R\x0ecloseTimestamp\"\x99\x04\n" +
	"\rKickCondition\x12R\n" +
	"\x10update_mutations\x18\x01 \x03(\v2'.room.members.private.v1.UpdateMutationR\x0fupdateMutations\x12_\n" +
	"\x17delete_stream_mutations\x18\x02 \x03(\v2'.room.streams.private.v1.DeleteMutationR\x15deleteStreamMutations\x12k\n" +
	"\x1bdelete_connection_mutations\x18\x03 \x03(\v2+.room.connections.private.v1.DeleteMutationR\x19deleteConnectionMutations\x12q\n" +
	"\x1cdelete_transponder_mutations\x18\x04 \x03(\v2/.gateway.transponders.private.v1.DeleteMutationR\x1adeleteTransponderMutations\x12s\n" +
	"\x1fdisconnect_connection_mutations\x18\x05 \x03(\v2+.gateway.sfus.private.v1.DisconnectMutationR\x1ddisconnectConnectionMutations\"V\n" +
	"\x0fKickTransaction\x12C\n" +
	"\tmutations\x18\x01 \x03(\v2%.room.members.private.v1.KickMutationR\tmutationsBDZBgitlab.com/evemeta/zdk/pb/out/go/room/members/private/v1;memberspbb\x06proto3"

var (
	file_room_members_private_v1_procedures_proto_rawDescOnce sync.Once
	file_room_members_private_v1_procedures_proto_rawDescData []byte
)

func file_room_members_private_v1_procedures_proto_rawDescGZIP() []byte {
	file_room_members_private_v1_procedures_proto_rawDescOnce.Do(func() {
		file_room_members_private_v1_procedures_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_room_members_private_v1_procedures_proto_rawDesc), len(file_room_members_private_v1_procedures_proto_rawDesc)))
	})
	return file_room_members_private_v1_procedures_proto_rawDescData
}

var file_room_members_private_v1_procedures_proto_msgTypes = make([]protoimpl.MessageInfo, 33)
var file_room_members_private_v1_procedures_proto_goTypes = []any{
	(*CountArgument)(nil),          // 0: room.members.private.v1.CountArgument
	(*CountRequest)(nil),           // 1: room.members.private.v1.CountRequest
	(*CountResponse)(nil),          // 2: room.members.private.v1.CountResponse
	(*RangeArgument)(nil),          // 3: room.members.private.v1.RangeArgument
	(*RangeRequest)(nil),           // 4: room.members.private.v1.RangeRequest
	(*RangeResponse)(nil),          // 5: room.members.private.v1.RangeResponse
	(*SelectArgument)(nil),         // 6: room.members.private.v1.SelectArgument
	(*SelectRequest)(nil),          // 7: room.members.private.v1.SelectRequest
	(*SelectResponse)(nil),         // 8: room.members.private.v1.SelectResponse
	(*CreateArgument)(nil),         // 9: room.members.private.v1.CreateArgument
	(*CreateRequest)(nil),          // 10: room.members.private.v1.CreateRequest
	(*CreateResponse)(nil),         // 11: room.members.private.v1.CreateResponse
	(*CreateMutation)(nil),         // 12: room.members.private.v1.CreateMutation
	(*CreateCondition)(nil),        // 13: room.members.private.v1.CreateCondition
	(*CreateTransaction)(nil),      // 14: room.members.private.v1.CreateTransaction
	(*UpdateArgument)(nil),         // 15: room.members.private.v1.UpdateArgument
	(*UpdateRequest)(nil),          // 16: room.members.private.v1.UpdateRequest
	(*UpdateResponse)(nil),         // 17: room.members.private.v1.UpdateResponse
	(*UpdateMutation)(nil),         // 18: room.members.private.v1.UpdateMutation
	(*UpdateTransaction)(nil),      // 19: room.members.private.v1.UpdateTransaction
	(*DeleteArgument)(nil),         // 20: room.members.private.v1.DeleteArgument
	(*DeleteRequest)(nil),          // 21: room.members.private.v1.DeleteRequest
	(*DeleteResponse)(nil),         // 22: room.members.private.v1.DeleteResponse
	(*DeleteMutation)(nil),         // 23: room.members.private.v1.DeleteMutation
	(*DeleteCondition)(nil),        // 24: room.members.private.v1.DeleteCondition
	(*DeleteTransaction)(nil),      // 25: room.members.private.v1.DeleteTransaction
	(*KickArgument)(nil),           // 26: room.members.private.v1.KickArgument
	(*KickRequest)(nil),            // 27: room.members.private.v1.KickRequest
	(*KickResponse)(nil),           // 28: room.members.private.v1.KickResponse
	(*KickMutation)(nil),           // 29: room.members.private.v1.KickMutation
	(*KickCondition)(nil),          // 30: room.members.private.v1.KickCondition
	(*KickTransaction)(nil),        // 31: room.members.private.v1.KickTransaction
	nil,                            // 32: room.members.private.v1.CreateArgument.MetadataEntry
	(*Query)(nil),                  // 33: room.members.private.v1.Query
	(*Chunk)(nil),                  // 34: room.members.private.v1.Chunk
	(*Member)(nil),                 // 35: room.members.private.v1.Member
	(Status)(0),                    // 36: room.members.private.v1.Status
	(Permission)(0),                // 37: room.members.private.v1.Permission
	(*Transient)(nil),              // 38: room.members.private.v1.Transient
	(*v1.CreateMutation)(nil),      // 39: gateway.transponders.private.v1.CreateMutation
	(*OptionalString)(nil),         // 40: room.members.private.v1.OptionalString
	(*OptionalMap)(nil),            // 41: room.members.private.v1.OptionalMap
	(*v1.DeleteMutation)(nil),      // 42: gateway.transponders.private.v1.DeleteMutation
	(*v11.DeleteMutation)(nil),     // 43: room.streams.private.v1.DeleteMutation
	(*v12.DeleteMutation)(nil),     // 44: room.connections.private.v1.DeleteMutation
	(*v13.DisconnectMutation)(nil), // 45: gateway.sfus.private.v1.DisconnectMutation
}
var file_room_members_private_v1_procedures_proto_depIdxs = []int32{
	33, // 0: room.members.private.v1.CountArgument.query:type_name -> room.members.private.v1.Query
	0,  // 1: room.members.private.v1.CountRequest.arguments:type_name -> room.members.private.v1.CountArgument
	33, // 2: room.members.private.v1.RangeArgument.query:type_name -> room.members.private.v1.Query
	3,  // 3: room.members.private.v1.RangeRequest.arguments:type_name -> room.members.private.v1.RangeArgument
	34, // 4: room.members.private.v1.RangeResponse.chunks:type_name -> room.members.private.v1.Chunk
	33, // 5: room.members.private.v1.SelectArgument.query:type_name -> room.members.private.v1.Query
	6,  // 6: room.members.private.v1.SelectRequest.arguments:type_name -> room.members.private.v1.SelectArgument
	35, // 7: room.members.private.v1.SelectResponse.members:type_name -> room.members.private.v1.Member
	36, // 8: room.members.private.v1.CreateArgument.status:type_name -> room.members.private.v1.Status
	37, // 9: room.members.private.v1.CreateArgument.permissions:type_name -> room.members.private.v1.Permission
	32, // 10: room.members.private.v1.CreateArgument.metadata:type_name -> room.members.private.v1.CreateArgument.MetadataEntry
	9,  // 11: room.members.private.v1.CreateRequest.arguments:type_name -> room.members.private.v1.CreateArgument
	35, // 12: room.members.private.v1.CreateResponse.members:type_name -> room.members.private.v1.Member
	9,  // 13: room.members.private.v1.CreateMutation.foundation:type_name -> room.members.private.v1.CreateArgument
	13, // 14: room.members.private.v1.CreateMutation.condition:type_name -> room.members.private.v1.CreateCondition
	38, // 15: room.members.private.v1.CreateMutation.transient:type_name -> room.members.private.v1.Transient
	39, // 16: room.members.private.v1.CreateCondition.create_transponder_mutations:type_name -> gateway.transponders.private.v1.CreateMutation
	12, // 17: room.members.private.v1.CreateTransaction.mutations:type_name -> room.members.private.v1.CreateMutation
	33, // 18: room.members.private.v1.UpdateArgument.query:type_name -> room.members.private.v1.Query
	40, // 19: room.members.private.v1.UpdateArgument.user_id:type_name -> room.members.private.v1.OptionalString
	40, // 20: room.members.private.v1.UpdateArgument.room_id:type_name -> room.members.private.v1.OptionalString
	41, // 21: room.members.private.v1.UpdateArgument.metadata:type_name -> room.members.private.v1.OptionalMap
	37, // 22: room.members.private.v1.UpdateArgument.permissions:type_name -> room.members.private.v1.Permission
	15, // 23: room.members.private.v1.UpdateRequest.arguments:type_name -> room.members.private.v1.UpdateArgument
	35, // 24: room.members.private.v1.UpdateResponse.members:type_name -> room.members.private.v1.Member
	15, // 25: room.members.private.v1.UpdateMutation.foundation:type_name -> room.members.private.v1.UpdateArgument
	38, // 26: room.members.private.v1.UpdateMutation.transient:type_name -> room.members.private.v1.Transient
	18, // 27: room.members.private.v1.UpdateTransaction.mutations:type_name -> room.members.private.v1.UpdateMutation
	33, // 28: room.members.private.v1.DeleteArgument.query:type_name -> room.members.private.v1.Query
	20, // 29: room.members.private.v1.DeleteRequest.arguments:type_name -> room.members.private.v1.DeleteArgument
	35, // 30: room.members.private.v1.DeleteResponse.members:type_name -> room.members.private.v1.Member
	20, // 31: room.members.private.v1.DeleteMutation.foundation:type_name -> room.members.private.v1.DeleteArgument
	24, // 32: room.members.private.v1.DeleteMutation.condition:type_name -> room.members.private.v1.DeleteCondition
	38, // 33: room.members.private.v1.DeleteMutation.transient:type_name -> room.members.private.v1.Transient
	42, // 34: room.members.private.v1.DeleteCondition.delete_transponder_mutations:type_name -> gateway.transponders.private.v1.DeleteMutation
	23, // 35: room.members.private.v1.DeleteTransaction.mutations:type_name -> room.members.private.v1.DeleteMutation
	33, // 36: room.members.private.v1.KickArgument.query:type_name -> room.members.private.v1.Query
	26, // 37: room.members.private.v1.KickRequest.arguments:type_name -> room.members.private.v1.KickArgument
	26, // 38: room.members.private.v1.KickMutation.foundation:type_name -> room.members.private.v1.KickArgument
	30, // 39: room.members.private.v1.KickMutation.condition:type_name -> room.members.private.v1.KickCondition
	18, // 40: room.members.private.v1.KickCondition.update_mutations:type_name -> room.members.private.v1.UpdateMutation
	43, // 41: room.members.private.v1.KickCondition.delete_stream_mutations:type_name -> room.streams.private.v1.DeleteMutation
	44, // 42: room.members.private.v1.KickCondition.delete_connection_mutations:type_name -> room.connections.private.v1.DeleteMutation
	42, // 43: room.members.private.v1.KickCondition.delete_transponder_mutations:type_name -> gateway.transponders.private.v1.DeleteMutation
	45, // 44: room.members.private.v1.KickCondition.disconnect_connection_mutations:type_name -> gateway.sfus.private.v1.DisconnectMutation
	29, // 45: room.members.private.v1.KickTransaction.mutations:type_name -> room.members.private.v1.KickMutation
	46, // [46:46] is the sub-list for method output_type
	46, // [46:46] is the sub-list for method input_type
	46, // [46:46] is the sub-list for extension type_name
	46, // [46:46] is the sub-list for extension extendee
	0,  // [0:46] is the sub-list for field type_name
}

func init() { file_room_members_private_v1_procedures_proto_init() }
func file_room_members_private_v1_procedures_proto_init() {
	if File_room_members_private_v1_procedures_proto != nil {
		return
	}
	file_room_members_private_v1_enums_proto_init()
	file_room_members_private_v1_entities_proto_init()
	file_room_members_private_v1_optionals_proto_init()
	file_room_members_private_v1_procedures_proto_msgTypes[15].OneofWrappers = []any{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_room_members_private_v1_procedures_proto_rawDesc), len(file_room_members_private_v1_procedures_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   33,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_room_members_private_v1_procedures_proto_goTypes,
		DependencyIndexes: file_room_members_private_v1_procedures_proto_depIdxs,
		MessageInfos:      file_room_members_private_v1_procedures_proto_msgTypes,
	}.Build()
	File_room_members_private_v1_procedures_proto = out.File
	file_room_members_private_v1_procedures_proto_goTypes = nil
	file_room_members_private_v1_procedures_proto_depIdxs = nil
}
