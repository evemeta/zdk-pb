...
class User$Type extends MessageType<User> {
    ...
    private binaryReadMap6(map: User["metadata"], reader: IBinaryReader, options: BinaryReadOptions): void {
        let len = reader.uint32(), end = reader.pos + len, key: keyof User["metadata"] | undefined, val: User["metadata"][any] | undefined;
        while (reader.pos < end) {
            let [fieldNo, wireType] = reader.tag();
            switch (fieldNo) {
                case 1:
                ...
                case 2:
                    val = reader.string();
                    break;
                default: throw new globalThis.Error("unknown map entry field for field user.users.public.server.v1.User.metadata");
            }
        }
        map[key ?? ""] = val ?? "";
    }
